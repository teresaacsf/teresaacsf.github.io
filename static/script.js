...
extractButton[1].addEventListener('click', () => {
  const storyTextIndex = 1;
  const max_ngram_size = getRandomInt(1, 10);
  extractKeywords(storyTextIndex, max_ngram_size);
});

extractButton[2].addEventListener('click', () => {
  const storyTextIndex = 2;
  const max_ngram_size = getRandomInt(1, 8);
  extractKeywords(storyTextIndex, max_ngram_size);
});
